...
# WATERSHED <- args[7]
# print(WATERSHED)
# WATERSHED <- as.numeric(WATERSHED)
# print(WATERSHED)
...
wsthreshold <- c(20, 25, 30, 35, 40, 45)
...
# parameters <- expand.grid(recon2 = recon2, recon3 = recon3)
...
for (i in 1:nrow(parameters)) {
  ...
  cmd$cmd5[i] <- paste0("recon-all -autorecon1 -s parameter-set-", i, " -sd ", 
                        subject.dir, " -skullstrip -smooth1 -smooth2 -gcut -norandomness -deface",
                        # " -multistrip",
                        " -wsthresh ", parameters[i,1],
                        " -subcortseg -clean-lta -no-wsgcaatlas -", parameters[i,2], 
                        " -", parameters[i,3])
                        # , " -parallel -openmp ", numCores)
...
c <- detectCores()
# c <- detectCores()/nrow(cmd)
registerDoMC(c)

# system("export WATERSHED_PREFLOOD_HEIGHTS='20 30 40 50'")
